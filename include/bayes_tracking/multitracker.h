...
namespace MTRK {
    ...
    struct observation_t {
        ...
        observation_t(FM::Vec v, double t, long unsigned i) : vec(v), time(t), id(i) {}
...
*/
    ...
    public:
         ...
         */
        void addObservation(const FM::Vec &z, double time, long unsigned id) {
            m_observations.push_back(observation_t(z, time, id));
        }
        ...
        template<class ObservationModelType>
        void process(ObservationModelType &om, association_t alg, std::map<long, int> &assignments ) {
            // data association
            if (dataAssociation(om, alg)) {
                // update
                observe(om);
            ...
            while (ui != m_unmatched.end()) {
                ...
                while (si != m_sequences.end()) {
                    if (m_observations[*ui].time - si->back().time > m_seqTime) { // erase old unmatched observations
                        ...
                        FilterType *filter;
                        if (si->size() >= m_seqSize &&
                            initialize(filter, *si)) {  // there's a minimum number of sequential observations
                            addFilter(filter);
                            // remove sequence
                            si = m_sequences.erase(si);
                            matched = true;
